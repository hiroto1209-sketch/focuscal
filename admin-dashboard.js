(() => {
  ...
  function score(x){return Number(x.priorityScore??((severityScore[x.severity]||20)+(categoryScore[x.category]||0)))}
  ...
  function card(x){const st=x.status||'new',s=score(x);return `<article class="card ${x.severity==='critical'?'critical':''}" data-id="${esc(x.id)}">
    <div class="card-top"><div class="badges"><span class="badge type">${labels[x.category]||x.category}</span><span class="badge sev ${esc(x.severity)}">${esc(x.severity)}</span><span class="badge status">${statuses[st]||st}</span><span class="score">P${s}</span></div><span class="time">${fmt(x.createdAt)}</span></div>
    ...
    <div class="meta">build ${esc(x.build||'-')} · ${esc(x.viewport||'-')} · ${esc(x.platform||'-')}</div>
    ...
    <textarea class="note" placeholder="開発メモ（例：v62で修正予定）">${esc(x.adminNote||'')}</textarea>
  </article>`}
  ...
  async function boot(){
    ...
    const [{initializeApp,getApps,getApp},{getAuth,signInAnonymously},{getFirestore,collection,onSnapshot,query,limit,doc,updateDoc,serverTimestamp,getDoc}]=await Promise.all([
    ...
    try{const admin=await getDoc(doc(db,'admins',uid));if(!admin.exists()){qs('#gate').classList.remove('hidden');qs('#dashboard').classList.add('hidden');return}}
    catch(e){qs('#gate').classList.remove('hidden');qs('#dashboard').classList.add('hidden');return}
    qs('#gate').classList.add('hidden');qs('#dashboard').classList.remove('hidden');
    onSnapshot(query(collection(db,'feedback'),limit(300)),snap=>{items=snap.docs.map(d=>({id:d.id,...d.data()}));render()},e=>{console.error(e);qs('#feedback-list').innerHTML='<div class="empty">Feedbackを取得できません。Rulesを確認してください。</div>'});
